...
const saveOptions = (): void => {
  const highlightElements = (document.getElementById('highlightElements') as HTMLInputElement).checked;
...
const restoreOptions = (): void => {
  chrome.storage.sync.get(
    ...
    (items: { highlightElements: boolean; useDB: boolean }) => {
      (document.getElementById('highlightElements') as HTMLInputElement).checked = items.highlightElements;
      (document.getElementById('useDB') as HTMLInputElement).checked = items.useDB;
    }
  );
};
