...
export class PageData {
    ...
    private getMatchingPatterns(): string[]{
        ...
        const matchingURLPatterns: string[] = Object.keys(this.urlPatternToSelectorData).filter((path) => {
            // console.log(path);
    ...
    private getSelectorNamePairs(matchingURLPatterns: string[]): SelectorNamePair[] {
        let currentSelectorNamePairs: SelectorNamePair[] = [];
        for (const urlPattern of matchingURLPatterns) {
            const selectorNamePairs: SelectorNamePair[] = this.urlPatternToSelectorData[urlPattern];
