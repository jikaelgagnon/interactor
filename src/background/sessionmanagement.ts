...
class TabSessionData {
  ...
  async flushActivitiesListToDB(useDB: boolean): Promise<void> {
    if (!useDB || !this.sessionId || this.activityList.length === 0) return
    if (!this.tabId) {
  ...
  async setSessionEndTimeInDB(useDB: boolean): Promise<void> {
    if (!useDB || !this.sessionId) return
...
class SessionManager {
   ...
   */
  private static async loadUseDB(): Promise<boolean> {
    try {
  ...
  public static async getInstance(): Promise<SessionManager> {
    if (!SessionManager.instance) {
      ...
      SessionManager.instance.useDB = await SessionManager.loadUseDB()
  ...
  private async flushCloseAndRemoveSession(tabId: number): Promise<void> {
    ...
    if (session) {
      await session.flushActivitiesListToDB(this.useDB)
      await session.setSessionEndTimeInDB(this.useDB)
