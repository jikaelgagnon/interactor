...
class DataManager {
  ...
  async getUserDataFromFirebase(): Promise<FirebaseSessionData[]> {
    ...
    try {
      ...
      console.log(
        `Retrieved ${userData.length} sessions for user:`,
        this.userEmail,
      )
      return userData
    } catch (error) {
  ...
  setupEventListeners(): void {
    ...
    const exportToJSONButton = document.getElementById(
      "exportToJson",
    ) as HTMLButtonElement
    if (exportToJSONButton) {
      const hasSessionsSelected = Array.from(this.selectedItems).some((id) =>
    ...
    const exportToXLSXButton = document.getElementById(
      "exportToXLSX",
    ) as HTMLButtonElement
  ...
  exportSessionsToXlsx(): void {
    ...
    const sessionsToExport = this.sessions.filter((session) =>
  ...
  createSessionElement(session: SessionData): HTMLElement {
    ...
    const endTime = session.sessionInfo.endTime
      ? new Date(session.sessionInfo.endTime).toLocaleString()
      : "Ongoing"
    ...
    if (session.activities.length > 0) {
      ...
      activitiesContainer.innerHTML =
        '<div class="no-activities">No activities in this session</div>'
    }
  ...
  updateUI(): void {
    ...
    const totalItems =
      ...
      this.sessions.reduce((sum, session) => sum + session.activities.length, 0)
    ...
    const sessionsToDelete = Array.from(this.selectedItems).filter((name) =>
    ...
    this.sessions.forEach((session) => {
      ...
      const sessionElement = document
        .getElementById(`session_${session.id}`)
        ?.closest(".session-group")
      if (sessionElement) {
        sessionElement.classList.toggle(
          "selected",
          this.selectedItems.has(`session_${session.id}`),
        )
      }
      ...
      session.activities.forEach((_, index) => {
        ...
        if (activityElement) {
          activityElement.classList.toggle(
            "selected",
            this.selectedItems.has(activityId),
          )
        }
      })
    })
  ...
  async deleteSelected(): Promise<void> {
    ...
    const sessionsToDelete = itemsToDelete.filter((name) =>
      name.startsWith("session_"),
    )
    ...
    try {
      ...
      if (deleteResult.success) {
        ...
        this.render()
        this.showStatus(
          `Successfully deleted ${sessionsToDelete.length} item(s)`,
        )
      } else {
   ...
   */
  async deleteUserDataFromFirebase(
    itemIds: string[],
    ...
    try {
      const sessionIds = itemIds
        .filter((name) => name.startsWith("session_"))
        .map((session_name) => session_name.split("_")[1])
      ...
      console.log(
        `Successfully processed deletion of ${sessionIds.length} items`,
      )
      return { success: true }
    } catch (error) {
