...
//   [key: string]: unknown; // Allows other properties on the activity
// }
...
//     async getUserDataFromFirebase(): Promise<object[]> {
//         if (!this.userEmail) {
...
class DataManager {
     ...
     * @returns An HTML element representing the activity
     */
    createActivityElement(activity: Activity, sessionId: string, index: number): HTMLElement {
        const activityId = `activity_${sessionId}_${index}`;
        const isSelected = this.selectedItems.has(activityId);
